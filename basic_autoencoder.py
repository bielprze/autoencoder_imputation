...
OUTPUT_DIR = Path("dae_results/scenario_A_empirical_outage_qc")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

MODEL_OUTPUT = OUTPUT_DIR / "dae_model.pt"
METRICS_OUTPUT = OUTPUT_DIR / "dae_metrics.csv"
HISTORY_OUTPUT = OUTPUT_DIR / "training_history.csv"
TEST_IMPUTED_OUTPUT = OUTPUT_DIR / "test_imputed_dae.csv"
...
def create_training_mask(
    ...
    """
    Creates artificial block missingness inside one training window.

    valid_target_mask:
        True  = this value is valid ground truth and can be masked
        False = QC invalid / original NaN, cannot be used in loss or masking
    """
...
class DenoisingWindowDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # Existing QC NaNs are not observed.
        # Artificially masked values are also not observed.
        observed_mask = valid_target_mask & (~artificial_missing_mask)

        # Neural network cannot receive NaN values.
        # We use 0.0 for missing/invalid values and provide observed_mask as second channel.
        ...
        input_features = np.concatenate(
            [
                corrupted.reshape(-1),
                observed_mask.astype(np.float32).reshape(-1),
            ],
            axis=0,
        ).astype(np.float32)
...
class DenoisingAutoencoder(nn.Module):
    def __init__(self, window_size: int, n_sensors: int):
        super().__init__()
...
def masked_mse_loss(
    ...
    """
    Loss ignores QC-invalid targets.

    Main loss:
        artificial missing values created during training.

    Auxiliary loss:
        observed valid values, with small weight.
    """
...
def compute_metrics(
    ...
    """
    Computes metrics only on valid artificial missing positions.

    Position is evaluated if:
    - mask == True,
    - y_true is finite,
    - y_pred is finite.
    """
...
def make_imputed_dataframe(
    ...
    """
    Fills only artificial missing values.

    QC-invalid NaNs remain NaN, because this project evaluates missing data
    imputation, not anomaly correction.
    """
...
def main() -> None:
    ...
    train_dataset = DenoisingWindowDataset(
